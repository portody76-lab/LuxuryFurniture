    ...
    <div class="bg-white rounded-2xl p-6 shadow">
        ...
        @if(request('search'))
            <div class="mb-4 flex items-center gap-2">
                ...
                <a href="{{ route('contents.operator.stock') }}" class="text-xs text-[#c9973a] hover:underline">Clear search</a>
            </div>
        @endif
    ...
    <div id="stockModal" class="modal-overlay">
        <div class="modal-animation bg-white rounded-3xl p-8 w-full max-w-md mx-4 shadow-2xl">
            <div class="text-center mb-4"><span
                    class="bg-[#c9973a] text-white text-xs font-bold tracking-wider px-5 py-1 rounded-full">STOCK
                    MANAGEMENT</span></div>
            <h2 id="modalTitle" class="font-playfair text-2xl font-bold text-center text-[#1a1208] mt-2 mb-6">Tambah Stok
            </h2>
            <form id="stockForm" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <div class="bg-[#f3e4c3] rounded-xl px-4 py-3"><span id="product_name"
                            class="text-sm text-[#3a3020]"></span></div>
                </div>
                <div class="mb-4">
                    <label class="block text-sm font-semibold text-[#3a3020] mb-2">Stok Saat Ini</label>
                    <div class="bg-gray-100 rounded-xl px-4 py-3"><span id="current_stock_display"
                            class="text-sm text-gray-600">0</span></div>
                </div>
                <div class="mb-4">
                ...
                <button type="submit" id="submitBtn"
                    class="w-full bg-[#c9973a] hover:bg-[#b07e28] text-white font-bold py-3 rounded-xl transition-all duration-200 hover:-translate-y-0.5">Proses</button>
            </form>
            <div class="text-center mt-4"><button onclick="closeModal('stockModal')"
                    class="text-[#9a8060] text-sm hover:text-[#7a5c1e] transition-colors">Tutup</button></div>
        </div>
    </div>
@endsection
    ...
    <script>
        ...
        function openStockModal(type, productId, productName, currentStock = 0) {
            ...
            if (type === 'add') {
                ...
                if (currentStock <= 0) { alert('Stok habis! Tidak bisa mengurangi stok.'); return; }
            }
        ...
        setTimeout(() => { const toast = document.getElementById('toast-box'); if (toast) setTimeout(() => toast.style.visibility = 'hidden', 3000); }, 100);
